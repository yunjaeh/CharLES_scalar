void CuttableVoronoiData::doit(const double L,const int * const ist_and_ipart_buf,const bool debug) {

  if (debug) {
    writeTecplot(-1);
    cout << "CuttableVoronoiData::doit: L: " << L << " nno_orig: " << nno << " ned_orig: " << ned << endl;
...
#undef IFA1
  ...
  if (intSet.size() == 1) {
	...
	continue;
      ...
      // we may need some fancier mempry management here...
      assert(nno <= nno_max);
	...
	if (faoed[ied][0] == X0_FACE) {
          assert(x_no_int[nooed[ied][0]][0] == -L_int);
          assert(x_no_int[nooed[ied][1]][0] == -L_int);
          //x_no_int[nooed[ied][0]][0] = -L_int-1;
	  //x_no_int[nooed[ied][1]][0] = -L_int-1;
	}
          ...
          assert(x_no_int[nooed[ied][0]][1] == -L_int);
	  assert(x_no_int[nooed[ied][1]][1] == -L_int);
	  //x_no_int[nooed[ied][0]][1] = -L_int-1;
	  //x_no_int[nooed[ied][1]][1] = -L_int-1;
	}
	  ...
	  assert(x_no_int[nooed[ied][0]][2] == -L_int);
	  assert(x_no_int[nooed[ied][1]][2] == -L_int);
	  //x_no_int[nooed[ied][0]][2] = -L_int-1;
	  //x_no_int[nooed[ied][1]][2] = -L_int-1;
	}
	else if (faoed[ied][0] == Z1_FACE) {
          assert(x_no_int[nooed[ied][0]][2] == L_int);
          assert(x_no_int[nooed[ied][1]][2] == L_int)1;
          //x_no_int[nooed[ied][0]][2] = L_int+1;
          //x_no_int[nooed[ied][1]][2] = L_int+1;
        }
      }
	...
	if (faoed[ied][0] == X0_FACE) {
      ...
      // for fast lookup, we will need and adt...
	...
	for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
      ...
      // do not delete bbmin_fa0,bbmax_fa0. We use it below for eliminating edge/face checks...
	...
	if (faceEdgeVec1[fe1].first != faceEdgeVec1[fe1-1].first)
      ...
      //cout << "got nfa1: " << nfa1 << endl;

      // =====================================================
      // the main loop is on the faces of part1...	
      // =====================================================
	...
	if (!intVec.empty()) {
	  for (int ii = 0; ii < intVec.size(); ++ii) {
	    ...
	    // start with the edges of ifa0 against face ifa1...
	    for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
	      const int ied0 = max(faceEdgeVec0[fe0].second,-faceEdgeVec0[fe0].second-1);
	      // only conside this edge if it is inside the bbox of ifa1...

	      if ((min(x_no_int[nooed[ied0][0]][0],x_no_int[nooed[ied0][1]][0]) <= bbmax_fa1[0])&&
		  (max(x_no_int[nooed[ied0][0]][0],x_no_int[nooed[ied0][1]][0]) >= bbmin_fa1[0])&&
		  (min(x_no_int[nooed[ied0][0]][1],x_no_int[nooed[ied0][1]][1]) <= bbmax_fa1[1])&&
		  (max(x_no_int[nooed[ied0][0]][1],x_no_int[nooed[ied0][1]][1]) >= bbmin_fa1[1])&&
		  (min(x_no_int[nooed[ied0][0]][2],x_no_int[nooed[ied0][1]][2]) <= bbmax_fa1[2])&&
		  (max(x_no_int[nooed[ied0][0]][2],x_no_int[nooed[ied0][1]][2]) >= bbmin_fa1[2])) {
		// if this edge is a cut-cube edge (i.e. on the cut cube surface), then we can use
		// 2D intersection routines...
		if (faoed[ied0][0] < 0) {

		  // TODO: take this out of the bbox check above and do faoed[ied0][0] < 0 check first!
		  ...
		  for (int fe1 = edofa_i1[ifa1]; fe1 != edofa_i1[ifa1+1]; ++fe1) {
		    if ((faceEdgeVec1[fe1].second < 0)&&(faoed[-faceEdgeVec1[fe1].second-1][0] == faoed[ied0][0])) {
		      const int ied1 = -faceEdgeVec1[fe1].second-1;
		...
		/*
		  else if (edofa_i1[ifa1+1] - edofa_i1[ifa1] == 3) {
		  // HEREHERE
		  cout << "XXXXXXXXX edge against tri!" << endl;
		  getchar();
		  }
		*/
		else {
		  assert(edofa_i1[ifa1+1] - edofa_i1[ifa1] >= 3);
		  // ied0 against ifa1 in 3D...
		  // build the face centroid...
		  double dist_x_fa1_sum[3] = { 0.0, 0.0, 0.0 };
		  double dist_sum = 0.0;
		  for (int fe1 = edofa_i1[ifa1]; fe1 != edofa_i1[ifa1+1]; ++fe1) {
		    const int ied1 = max(faceEdgeVec1[fe1].second,-faceEdgeVec1[fe1].second-1);
		    const double dist = DIST(x_no[nooed[ied1][0]],x_no[nooed[ied1][1]]);
		    FOR_I3 dist_x_fa1_sum[i] += dist*(x_no[nooed[ied1][0]][i]+x_no[nooed[ied1][1]][i]);
		    dist_sum += dist;
		  }
		  if (!(dist_sum != 0.0)) 
                    throw(-1);
		  assert(dist_sum != 0.0);
		  dist_sum = 0.5/dist_sum;
		  int x_fa1_int[3]; FOR_I3 x_fa1_int[i] = (int)floor(dist_x_fa1_sum[i]*dist_sum/L*double(L_int)+0.5);
		  // now look for an edge that is on either side of the tris formed
		  // by looping through the edges...
		  for (int fe1 = edofa_i1[ifa1]; fe1 != edofa_i1[ifa1+1]; ++fe1) {
		    int ino0,ino1;
		    if (faceEdgeVec1[fe1].second >= 0) {
		      // faceEdgeVec1[fe1].second stores the edge index normally...
		      ino0 = nooed[faceEdgeVec1[fe1].second][0];
		      ino1 = nooed[faceEdgeVec1[fe1].second][1];
		    }
		    else {
		      // faceEdgeVec1[fe1].second stores the edge index using -1 indexing...
		      ino0 = nooed[-faceEdgeVec1[fe1].second-1][1];
		      ino1 = nooed[-faceEdgeVec1[fe1].second-1][0];
		    }
		    const int8 tet_vol0 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied0][0]],x_fa1_int,x_no_int[ino0],x_no_int[ino1]);
		    const int8 tet_vol1 = SIGNED_TET_VOLUME_6_INT8(x_fa1_int,x_no_int[nooed[ied0][1]],x_no_int[ino0],x_no_int[ino1]);
		    if (tet_vol0+tet_vol1 == 0) {
		      // skip zeros...
		      // TODO: look into these some day?...
		      if (debug) {
			cout << "Warning: skipping zero L1484: ifa1: " << ifa1 << " ied0: " << ied0 << endl;
			getchar();
			/*
			...
			*/
		      ...
		      const int8 tet_vol_ed1 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied0][0]],x_no_int[nooed[ied0][1]],x_no_int[ino0],x_no_int[ino1]);
		      if ((tet_vol_ed1 >= 0)&&(tet_vol_ed1 <= tet_vol0+tet_vol1)) {
			// need to check other volumes associated with the internal edges...
			const int8 tet_vol_ie0 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied0][0]],x_no_int[nooed[ied0][1]],x_fa1_int,x_no_int[ino0]);
			if ((tet_vol_ie0 >= 0)&&(tet_vol_ed1+tet_vol_ie0 <= tet_vol0+tet_vol1)) {
			  const int8 tet_vol_ie1 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied0][0]],x_no_int[nooed[ied0][1]],x_no_int[ino1],x_fa1_int);
			  if (tet_vol_ie1 >= 0) {
			    // got it!...
			    assert(tet_vol_ed1+tet_vol_ie0+tet_vol_ie1 == tet_vol0+tet_vol1);
			    // a few possibilities here now...
			    if (tet_vol_ed1 > 0) {
			      // even if either of the internal edges is zero, this is still a edge-face intersection...
			      if (tet_vol0 == 0) {
				assert(tet_vol1 > 0);
				// note: faceEdgeVec1[fe1].first contains the original faoed for ifa1...
				localIntersectionVec.push_back(IntersectionData(NODE_FACE_INTERSECTION,nooed[ied0][0],faceEdgeVec1[fe1].first));
			      }
			      else if (tet_vol1 == 0) {
				assert(tet_vol0 > 0);
				// note: faceEdgeVec1[fe1].first contains the original faoed for ifa1...
				localIntersectionVec.push_back(IntersectionData(NODE_FACE_INTERSECTION,nooed[ied0][1],faceEdgeVec1[fe1].first));
			      }
			      else {
				assert(tet_vol0 > 0);
				assert(tet_vol1 > 0);
				// note: faceEdgeVec1[fe1].first contains the original faoed for ifa1...
				localIntersectionVec.push_back(IntersectionData(EDGE_FACE_INTERSECTION,ied0,faceEdgeVec1[fe1].first,
										double(tet_vol0)/double(tet_vol0+tet_vol1)));
			      }
			    }
			    else {
			      // edge-edge or edge-node, etc... 
			      assert(tet_vol_ed1 == 0);
			      if (tet_vol0 == 0) {
				// we are at ino0 of ied0...
				assert(tet_vol1 > 0);
				assert(tet_vol_ie0 > 0);
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec1[fe1].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied0][0],faceEdgeVec1[fe1].second,
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied0][0],-faceEdgeVec1[fe1].second-1,
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
			      else if (tet_vol1 == 0) {
				...
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec1[fe1].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied0][1],faceEdgeVec1[fe1].second,
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied0][1],-faceEdgeVec1[fe1].second-1,
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
			      else {
				...
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec1[fe1].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(EDGE_EDGE_INTERSECTION,ied0,faceEdgeVec1[fe1].second,
										  double(tet_vol0)/double(tet_vol0+tet_vol1),
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(EDGE_EDGE_INTERSECTION,ied0,-faceEdgeVec1[fe1].second-1,
										  double(tet_vol0)/double(tet_vol0+tet_vol1),
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
			...
			if ((tet_vol_ie0 <= 0)&&(tet_vol_ed1+tet_vol_ie0 >= tet_vol0+tet_vol1)) {
			  ...
			  if (tet_vol_ie1 <= 0) {
			    ...
			    // a few possibilities here now...
			    if (tet_vol_ed1 < 0) {
			      // even if either of the internal edges is zero, this is still a face-edge intersection...
			      if (tet_vol0 == 0) {
				assert(tet_vol1 < 0);
				localIntersectionVec.push_back(IntersectionData(NODE_FACE_INTERSECTION,nooed[ied0][0],faceEdgeVec1[fe1].first));
			      }
				...
				assert(tet_vol1 < 0);
				// note: faceEdgeVec1[fe1].first contains the original faoed for ifa1...
				localIntersectionVec.push_back(IntersectionData(EDGE_FACE_INTERSECTION,ied0,faceEdgeVec1[fe1].first,double(tet_vol0)/double(tet_vol0+tet_vol1)));
			      }
			    }
			    else {
			      // edge-edge or edge-node, etc... 
	    ...
	    // ==================================================
	    // second: edges of ifa1 against face ifa0...
	    // ==================================================
	    for (int fe1 = edofa_i1[ifa1]; fe1 != edofa_i1[ifa1+1]; ++fe1) {
	      const int ied1 = max(faceEdgeVec1[fe1].second,-faceEdgeVec1[fe1].second-1);
	      // only conside this edge if it is inside the bbox of ifa0...
	      if ((min(x_no_int[nooed[ied1][0]][0],x_no_int[nooed[ied1][1]][0]) <= bbmax_fa0[ifa0][0])&&
		  (max(x_no_int[nooed[ied1][0]][0],x_no_int[nooed[ied1][1]][0]) >= bbmin_fa0[ifa0][0])&&
		  (min(x_no_int[nooed[ied1][0]][1],x_no_int[nooed[ied1][1]][1]) <= bbmax_fa0[ifa0][1])&&
		  (max(x_no_int[nooed[ied1][0]][1],x_no_int[nooed[ied1][1]][1]) >= bbmin_fa0[ifa0][1])&&
		  (min(x_no_int[nooed[ied1][0]][2],x_no_int[nooed[ied1][1]][2]) <= bbmax_fa0[ifa0][2])&&
		  (max(x_no_int[nooed[ied1][0]][2],x_no_int[nooed[ied1][1]][2]) >= bbmin_fa0[ifa0][2])) {
		// no need to reconsider the 2D case here. Only need to handle once (above)...
		if (faoed[ied1][0] >= 0) {
		  // ied1 against ifa0 in 3D...

		  // HEREHERE: could do edge tri specifically without building face centroid...
		  if (edofa_i0[ifa0+1] - edofa_i0[ifa0] == 3) {
		    //if (debug) cout << "L1707 TRI!" << endl;
		  }
		  else {
		    assert(edofa_i0[ifa0+1] - edofa_i0[ifa0] > 3);
		  }

		  // build the face centroid...
		  double dist_x_fa0_sum[3] = { 0.0, 0.0, 0.0 };
		  double dist_sum = 0.0;
		  for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
		    const int ied0 = max(faceEdgeVec0[fe0].second,-faceEdgeVec0[fe0].second-1);
		    const double dist = DIST(x_no[nooed[ied0][0]],x_no[nooed[ied0][1]]);
		    FOR_I3 dist_x_fa0_sum[i] += dist*(x_no[nooed[ied0][0]][i]+x_no[nooed[ied0][1]][i]);
		    dist_sum += dist;
		  }
		  assert(dist_sum != 0.0);
		  dist_sum = 0.5/dist_sum;
		  int x_fa0_int[3]; FOR_I3 x_fa0_int[i] = (int)floor(dist_x_fa0_sum[i]*dist_sum/L*double(L_int)+0.5);
		  // now look for an edge that is on either side of the tris formed
		  // by looping through the edges...
		  for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
		    int ino0,ino1;
		    if (faceEdgeVec0[fe0].second >= 0) {
		      // faceEdgeVec0[fe0].second stores the edge index normally...
		      ino0 = nooed[faceEdgeVec0[fe0].second][0];
		      ino1 = nooed[faceEdgeVec0[fe0].second][1];
		    }
		    else {
		      // faceEdgeVec0[fe0].second stores the edge index using -1 indexing...
		      ino0 = nooed[-faceEdgeVec0[fe0].second-1][1];
		      ino1 = nooed[-faceEdgeVec0[fe0].second-1][0];
		    }
		    const int8 tet_vol0 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied1][0]],x_fa0_int,x_no_int[ino0],x_no_int[ino1]);
		    const int8 tet_vol1 = SIGNED_TET_VOLUME_6_INT8(x_fa0_int,x_no_int[nooed[ied1][1]],x_no_int[ino0],x_no_int[ino1]);
		    if (tet_vol0+tet_vol1 == 0) {
		      // skip zeros...
		      // TODO: look into these some day?...
		      if (debug) {
			cout << "*************** Warning: skipping zero L1576: ifa0: " << ifa0 << " ied1: " << ied1 << " ****************" << endl;
			//getchar();
			/*
			  FILE * fp = fopen("face.dat","w");
			  for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
			...
			*/
		      ...
		      // this is a potenially a positive crossing of the edge...
		      const int8 tet_vol_ed1 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied1][0]],x_no_int[nooed[ied1][1]],x_no_int[ino0],x_no_int[ino1]);
		      if ((tet_vol_ed1 >= 0)&&(tet_vol_ed1 <= tet_vol0+tet_vol1)) {
			// need to check other volumes associated with the internal edges...
			const int8 tet_vol_ie0 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied1][0]],x_no_int[nooed[ied1][1]],x_fa0_int,x_no_int[ino0]);
			if ((tet_vol_ie0 >= 0)&&(tet_vol_ed1+tet_vol_ie0 <= tet_vol0+tet_vol1)) {
			  const int8 tet_vol_ie1 = SIGNED_TET_VOLUME_6_INT8(x_no_int[nooed[ied1][0]],x_no_int[nooed[ied1][1]],x_no_int[ino1],x_fa0_int);
			  if (tet_vol_ie1 >= 0) {
			    // got it!...
			    assert(tet_vol_ed1+tet_vol_ie0+tet_vol_ie1 == tet_vol0+tet_vol1);
			    // a few possibilities here now...
			    if (tet_vol_ed1 > 0) {
			      ...
			      if (tet_vol0 == 0) {
				assert(tet_vol1 > 0);
				localIntersectionVec.push_back(IntersectionData(NODE_FACE_INTERSECTION,nooed[ied1][0],faceEdgeVec0[fe0].first));
			      }
			      else if (tet_vol1 == 0) {
				assert(tet_vol0 > 0);
				localIntersectionVec.push_back(IntersectionData(NODE_FACE_INTERSECTION,nooed[ied1][1],faceEdgeVec0[fe0].first));
			      }
			      else {
				assert(tet_vol0 > 0);
				assert(tet_vol1 > 0);
				localIntersectionVec.push_back(IntersectionData(EDGE_FACE_INTERSECTION,ied1,faceEdgeVec0[fe0].first,double(tet_vol0)/double(tet_vol0+tet_vol1)));
			      }
			    }
			    else {
			      // edge-edge or edge-node, etc... 
			      assert(tet_vol_ed1 == 0);
			      if (tet_vol0 == 0) {
				// we are at ino0 of ied1...
				assert(tet_vol1 > 0);
				assert(tet_vol_ie0 > 0);
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec0[fe0].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied1][0],faceEdgeVec0[fe0].second,
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied1][0],-faceEdgeVec0[fe0].second-1,
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
			      else if (tet_vol1 == 0) {
				// we are at ino1 of ied1...
				assert(tet_vol0 > 0);
				assert(tet_vol_ie0 > 0);
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec0[fe0].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied1][1],faceEdgeVec0[fe0].second,
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(NODE_EDGE_INTERSECTION,nooed[ied1][1],-faceEdgeVec0[fe0].second-1,
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
			      else {
				...
				assert(tet_vol_ie1 > 0);
				if (faceEdgeVec0[fe0].second >= 0) {
				  localIntersectionVec.push_back(IntersectionData(EDGE_EDGE_INTERSECTION,ied1,faceEdgeVec0[fe0].second,
										  double(tet_vol0)/double(tet_vol0+tet_vol1),
										  double(tet_vol_ie0)/double(tet_vol_ie0+tet_vol_ie1)));
				}
				else {
				  localIntersectionVec.push_back(IntersectionData(EDGE_EDGE_INTERSECTION,ied1,-faceEdgeVec0[fe0].second-1,
										  double(tet_vol0)/double(tet_vol0+tet_vol1),
										  double(tet_vol_ie1)/double(tet_vol_ie0+tet_vol_ie1)));
				}
			      }
	    ...
	    //cout << "ifa0,ifa1: " << ifa0 << " " << ifa1 << " localIntersectionVec.size() : " << localIntersectionVec.size() << endl;
	    if (localIntersectionVec.size() >= 2) {
	      sort(localIntersectionVec.begin(),localIntersectionVec.end());
	      vector<pair<double,int> > diVec;
	      if (debug) cout << "INTERSECTIONS between faces ifa0: " << ifa0 << " AND ifa1: " << ifa1 << endl;
		...
		if ((ii == 0)||(localIntersectionVec[ii] != localIntersectionVec[ii-1])) {
		  ...
		  // NODE_NODE HACK...
		  if (localIntersectionVec[ii].kind == NODE_NODE_INTERSECTION) {
		    cout << "AAAAAAAAAAAAAAAAAAAAAA" << endl;
		    intersectionVec.push_back(localIntersectionVec[ii]);
		  }
		}
	      }
		...
		fclose(fp);
		cout << "XXXXXXXXXXXXXX take a look" << endl;
		getchar();
		}
		}
	      */
	      
	      // 2 or more intersections means there may be some lines to draw...
	      if (diVec.size() >= 2) {
		...
		double normal_fa0[3] = { 0.0, 0.0, 0.0 };
		int ino_; // for storing the starting node...
		for (int fe0 = edofa_i0[ifa0]; fe0 != edofa_i0[ifa0+1]; ++fe0) {
		  if (faceEdgeVec0[fe0].second >= 0) {
		    if (fe0 == edofa_i0[ifa0]) {
		      ino_ = nooed[faceEdgeVec0[fe0].second][0];
		    }
		    else {
		      const int ino0 = nooed[faceEdgeVec0[fe0].second][0];
		      const int ino1 = nooed[faceEdgeVec0[fe0].second][1];
		      const int8 this_normal[3] = TRI_NORMAL_2_INT8(x_no_int[ino_],x_no_int[ino0],x_no_int[ino1]);
		      FOR_I3 normal_fa0[i] += (double)this_normal[i];
		    }
		  }
		  else {
		    if (fe0 == edofa_i0[ifa0]) {
		      ino_ = nooed[-faceEdgeVec0[fe0].second-1][1];
		    }
		    else {
		      const int ino0 = nooed[-faceEdgeVec0[fe0].second-1][1];
		      const int ino1 = nooed[-faceEdgeVec0[fe0].second-1][0];
		      const int8 this_normal[3] = TRI_NORMAL_2_INT8(x_no_int[ino_],x_no_int[ino0],x_no_int[ino1]);
		      FOR_I3 normal_fa0[i] += (double)this_normal[i];
		    }
		  }
		}
		double normal_fa1[3] = { 0.0, 0.0, 0.0 };
		for (int fe1 = edofa_i1[ifa1]; fe1 != edofa_i1[ifa1+1]; ++fe1) {
		  if (faceEdgeVec1[fe1].second >= 0) {
		    if (fe1 == edofa_i1[ifa1]) {
		      ino_ = nooed[faceEdgeVec1[fe1].second][0];
		    }
		    else {
		      const int ino0 = nooed[faceEdgeVec1[fe1].second][0];
		      const int ino1 = nooed[faceEdgeVec1[fe1].second][1];
		      const int8 this_normal[3] = TRI_NORMAL_2_INT8(x_no_int[ino_],x_no_int[ino0],x_no_int[ino1]);
		      FOR_I3 normal_fa1[i] += this_normal[i];
		    }
		  }
		  else {
		    if (fe1 == edofa_i1[ifa1]) {
		      ino_ = nooed[-faceEdgeVec1[fe1].second-1][1];
		    }
		    else {
		      const int ino0 = nooed[-faceEdgeVec1[fe1].second-1][1];
		      const int ino1 = nooed[-faceEdgeVec1[fe1].second-1][0];
		      const int8 this_normal[3] = TRI_NORMAL_2_INT8(x_no_int[ino_],x_no_int[ino0],x_no_int[ino1]);
		      FOR_I3 normal_fa1[i] += this_normal[i];
		    }
		  }
		...
		const double cp[3] = CROSS_PRODUCT(normal_fa0,normal_fa1);
		...
		for (int ii = 0; ii < diVec.size(); ++ii) {
		  ...
		  diVec[ii].first = DOT_PRODUCT(xi,cp);
		}
		sort(diVec.begin(),diVec.end());
		
		// connect the two furthest together here...
		const int ied = new_edge();
		faoed[ied][0] = faceEdgeVec0[edofa_i0[ifa0]].first;
		faoed[ied][1] = faceEdgeVec1[edofa_i1[ifa1]].first;
		...
		if (diVec.size() > 2) cout << "XXXXXXX diVec.size(): " << diVec.size() << endl;
      ...
      //cout << "XXXXXXXXXXXXX Done face-face for ipart1: " << ipart1 << " intersectionVec.size(): " << intersectionVec.size() << " ned-ned_old: " << ned-ned_old << endl;
    ...
    // inconsistent with the face orientation set by the iontersections, and thus out...
    ...
    for (int ii = diVec.size()-1; ii >= 0; --ii) {
      const int ied = diVec[ii].second;
	...
	// orientation above (during the dp check). Try flipping faoed...
	if (debug) {
	  cout << "intersection edge orientation FAILURE: trying flip logic..." << endl;
	}
	const int tmp = faoed[ied][0];
	faoed[ied][0] = faoed[ied][1];
	faoed[ied][1] = tmp;
	f0 = (forwardFaNoSet.find(pair<int,int>(faoed[ied][0],nooed[ied][0])) == forwardFaNoSet.end());
	f1 = (forwardFaNoSet.find(pair<int,int>(faoed[ied][1],nooed[ied][1])) == forwardFaNoSet.end());
	b0 = (backwardFaNoSet.find(pair<int,int>(faoed[ied][0],nooed[ied][1])) == backwardFaNoSet.end());
	b1 = (backwardFaNoSet.find(pair<int,int>(faoed[ied][1],nooed[ied][0])) == backwardFaNoSet.end());
	if (f0 && f1 && b0 && b1) {
	  forwardFaNoSet.insert(pair<int,int>(faoed[ied][0],nooed[ied][0]));
	  forwardFaNoSet.insert(pair<int,int>(faoed[ied][1],nooed[ied][1]));
	  backwardFaNoSet.insert(pair<int,int>(faoed[ied][0],nooed[ied][1]));
	  ...
	  cout << "Error: L2077: f0: " << f0 << " f1: " << f1 << " b0: " << b0 << " b1: " << b1 << endl;
	  throw(-1);
	}
      }
    }
