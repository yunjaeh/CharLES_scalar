...
StFlowLe::StFlowLe(IdealGasPhase* ph, size_t nsp, size_t points) :
...
void StFlowLe::resize(size_t ncomponents, size_t points)
{
...
//    m_Le.resize(m_nsp, 0.0);

    if (m_transport_option ==  c_Mixav_Transport_Le) {
        m_diff.resize(m_nsp*m_points);
    } else {
        m_multidiff.resize(m_nsp*m_nsp*m_points);
        m_diff.resize(m_nsp*m_points);
...
void StFlowLe::setupGrid(size_t n, const doublereal* z)
{
    resize(m_nv, n);
    size_t j;
    ...
    for (j = 1; j < m_points; j++) {
        if (z[j] <= z[j-1]) {
            throw CanteraError("StFlowLe::setupGrid",
...
void StFlowLe::setTransport(Transport& trans, const double* Le, bool withSoret)
{
    m_trans = &trans;
...
#ifdef OLD_CANTERA
    string model;
    int i_model = m_trans->model();
    if (i_model == cMulticomponent || i_model == CK_Multicomponent)
      model = "Multi"; 
    else if (i_model == cMixtureAveraged || i_model == CK_MixtureAveraged)
      model = "Mix"; 
    else
      model = "unknown"; 
#else
    string model = m_trans->transportType();
#endif

    if (model == "Multi") {
        m_transport_option = c_Multi_Transport_Le;
        m_multidiff.resize(m_nsp*m_nsp*m_points);
        m_diff.resize(m_nsp*m_points);
        m_dthermal.resize(m_nsp, m_points, 0.0);
    } else if (model == "Mix") {
        m_transport_option = c_Mixav_Transport_Le;
        m_diff.resize(m_nsp*m_points);
        if (withSoret)
...
//    // XXX ars
    m_Le.resize(m_nsp);
//    m_Le = new double[m_nsp];
    for (int isp=0; isp<m_nsp; isp++) {
      m_Le[isp] = Le[isp];
//      cout << "isp = " << isp << "; Le = " << m_Le[isp] << endl;
    }
}

void StFlowLe::enableSoret(bool withSoret)
{
    if (m_transport_option == c_Multi_Transport_Le) {
        m_do_soret = withSoret;
    } else {
        throw CanteraError("setTransport",
                           "Thermal diffusion (the Soret effect) "
...
void StFlowLe::setGas(const doublereal* x, size_t j)
{
    m_thermo->setTemperature(T(x,j));
    const doublereal* yy = x + m_nv*j + c_offset_Y_Le;
    m_thermo->setMassFractions_NoNorm(yy);
    m_thermo->setPressure(m_press);
}

void StFlowLe::setGasAtMidpoint(const doublereal* x, size_t j)
{
    m_thermo->setTemperature(0.5*(T(x,j)+T(x,j+1)));
    const doublereal* yyj = x + m_nv*j + c_offset_Y_Le;
    const doublereal* yyjp = x + m_nv*(j+1) + c_offset_Y_Le;
    for (size_t k = 0; k < m_nsp; k++) {
        m_ybar[k] = 0.5*(yyj[k] + yyjp[k]);
    }
    m_thermo->setMassFractions_NoNorm(&m_ybar[0]);
    m_thermo->setPressure(m_press);
}

void StFlowLe::_finalize(const doublereal* x)
{
    size_t k, j;
    doublereal zz, tt;
...
void StFlowLe::eval(size_t jg, doublereal* xg,
                  doublereal* rg, integer* diagg, doublereal rdt)
{
    ...
    for (j = jmin; j <= jmax; j++) {
        ...
        if (j == 0) {
            ...
            // the boundary object
            rsd[index(c_offset_V_Le,0)] = V(x,0);
            rsd[index(c_offset_T_Le,0)] = T(x,0);
            rsd[index(c_offset_L_Le,0)] = -rho_u(x,0);
            ...
            for (k = 0; k < m_nsp; k++) {
                sum += Y(x,k,0);
                rsd[index(c_offset_Y_Le + k, 0)] =
                    -(m_flux(k,0) + rho_u(x,0)* Y(x,k,0));
            }
            rsd[index(c_offset_Y_Le, 0)] = 1.0 - sum;
        }
            ...
            //
            //-------------------------------------------------
            rsd[index(c_offset_V_Le,j)]
            = (shear(x,j) - lambda(x,j) - rho_u(x,j)*dVdz(x,j)
               - m_rho[j]*V(x,j)*V(x,j))/m_rho[j]
              - rdt*(V(x,j) - V_prev(j));
            diag[index(c_offset_V_Le, j)] = 1;
            ...
            for (k = 0; k < m_nsp; k++) {
                ...
                diffus = 2.0*(m_flux(k,j) - m_flux(k,j-1))
                         /(z(j+1) - z(j-1));
                rsd[index(c_offset_Y_Le + k, j)]
                = (m_wt[k]*(wdot(k,j))
                   - convec - diffus)/m_rho[j]
                  - rdt*(Y(x,k,j) - Y_prev(k,j));
                diag[index(c_offset_Y_Le + k, j)] = 1;
            }
            ...
            if (m_do_energy[j]) {
                ...
                rsd[index(c_offset_T_Le, j)]   =
                    - m_cp[j]*rho_u(x,j)*dtdzj
                    - divHeatFlux(x,j) - sum - sum2;
                rsd[index(c_offset_T_Le, j)] /= (m_rho[j]*m_cp[j]);

                rsd[index(c_offset_T_Le, j)] -= rdt*(T(x,j) - T_prev(j));
                rsd[index(c_offset_T_Le, j)] -= (m_qdotRadiation[j] / (m_rho[j] * m_cp[j]));
                diag[index(c_offset_T_Le, j)] = 1;
            } else {
                // residual equations if the energy equation is disabled
                rsd[index(c_offset_T_Le, j)] = T(x,j) - T_fixed(j);
                diag[index(c_offset_T_Le, j)] = 0;
            }

            rsd[index(c_offset_L_Le, j)] = lambda(x,j) - lambda(x,j-1);
            diag[index(c_offset_L_Le, j)] = 0;
        }
    }
}

void StFlowLe::updateTransport(doublereal* x, size_t j0, size_t j1)
{
    if (m_transport_option == c_Mixav_Transport_Le) {
        for (size_t j = j0; j < j1; j++) {
            setGasAtMidpoint(x,j);
            m_visc[j] = (m_dovisc ? m_trans->viscosity() : 0.0);
            m_trans->getMixDiffCoeffs(&m_diff[0] + j*m_nsp);
            m_tcon[j] = m_trans->thermalConductivity();
        }
    } else if (m_transport_option == c_Multi_Transport_Le) {
        for (size_t j = j0; j < j1; j++) {
            setGasAtMidpoint(x,j);
...
void StFlowLe::showSolution(const doublereal* x)
{
    size_t nn = m_nv/5;
    size_t i, j, n;
    char buf[100];
...
void StFlowLe::updateDiffFluxes(const doublereal* x, size_t j0, size_t j1)
{
    size_t j, k, m;
        ...
        for (j = j0; j < j1; j++) {
            ...
            for (k = 0; k < m_nsp; k++) {
                ...
                if (m_Le[k] <= 0) {
                  ...
                  double Dk = m_tcon[j] / (rho*m_cp[j]*m_Le[k]);
                  m_flux(k,j) = m_wt[k]*(rho*Dk/wtm);
                }
...
string StFlowLe::componentName(size_t n) const
{
    switch (n) {
    case 0:
        return "u";
    ...
    case 3:
        return "lambda";
    default:
        if (n >= c_offset_Y_Le && n < (c_offset_Y_Le + m_nsp)) {
            return m_thermo->speciesName(n - c_offset_Y_Le);
        } else {
            return "<unknown>";
        }
    }
...
size_t StFlowLe::componentIndex(const std::string& name) const
{
    if (name=="u") {
        return 0;
    } else if (name=="V") {
...
void StFlowLe::restore(const XML_Node& dom, doublereal* soln, int loglevel)
{
    ...
    for (n = 0; n < nd; n++) {
        ...
        if (nm == "z") {
            ...
            np = x.size();
            writelog("Grid contains "+std::to_string(np)+" points.\n", loglevel >= 2);
            readgrid = true;
            setupGrid(np, &x[0]);
        }
    }
    if (!readgrid) {
        throw CanteraError("StFlowLe::restore",
    ...
    if (dom.hasChild("energy_enabled")) {
        ...
        if (x.size() == nPoints()) {
            ...
            throw CanteraError("StFlowLe::restore", "energy_enabled is length" +
                               std::to_string(x.size()) + "but should be length" +
    ...
    if (dom.hasChild("species_enabled")) {
        ...
        if (x.size() == m_nsp) {
            ...
            // number of species.
            if (loglevel > 0) {
                writelog("\nWarning: StFlowLe::restore: species_enabled is length " +
                         std::to_string(x.size()) + " but should be length " +
                         std::to_string(m_nsp) + ". Enabling all species equations by default.");
            }
...
XML_Node& StFlowLe::save(XML_Node& o, const doublereal* const sol)
{
    size_t k;
    ...
    addFloatArray(gv,"z",m_z.size(),&m_z[0],
                  "m","length");
    vector_fp x(soln.nColumns());

    soln.getRow(0,&x[0]);
    addFloatArray(gv,"u",x.size(),&x[0],"m/s","velocity");
    ...
    soln.getRow(2,&x[0]);
    addFloatArray(gv,"T",x.size(),&x[0],"K","temperature");

    soln.getRow(3,&x[0]);
    addFloatArray(gv,"L",x.size(),&x[0],"N/m^4");

    for (k = 0; k < m_nsp; k++) {
        soln.getRow(4+k,&x[0]);
        addFloatArray(gv,m_thermo->speciesName(k),
                      x.size(),&x[0],"","massFraction");
    }
    if (m_do_radiation) {
        addFloatArray(gv, "radiative_heat_loss", m_z.size(),
            &m_qdotRadiation[0], "W/m^3", "specificPower");
    }
...
void StFlowLe::setJac(MultiJac* jac)
{
    m_jac = jac;
}

void AxiStagnFlowLe::evalRightBoundary(doublereal* x, doublereal* rsd,
...
void AxiStagnFlowLe::evalContinuity(size_t j, doublereal* x, doublereal* rsd,
                                  integer* diag, doublereal rdt)
{
    //----------------------------------------------
    //    Continuity equation
    ...
    rsd[index(c_offset_U_Le,j)] =
        -(rho_u(x,j+1) - rho_u(x,j))/m_dz[j]
        -(density(j+1)*V(x,j+1) + density(j)*V(x,j));

    //algebraic constraint
    diag[index(c_offset_U_Le, j)] = 0;
}

FreeFlameLe::FreeFlameLe(IdealGasPhase* ph, size_t nsp, size_t points) :
...
void FreeFlameLe::evalRightBoundary(doublereal* x, doublereal* rsd,
                                  integer* diag, doublereal rdt)
{
    size_t j = m_points - 1;
...
void FreeFlameLe::evalContinuity(size_t j, doublereal* x, doublereal* rsd,
                               integer* diag, doublereal rdt)
{
    //----------------------------------------------
    //    Continuity equation
    ...
    if (grid(j) > m_zfixed) {
        ...
        if (m_do_energy[j]) {
            rsd[index(c_offset_U_Le,j)] = (T(x,j) - m_tfixed);
        } else {
            rsd[index(c_offset_U_Le,j)] = (rho_u(x,j)
                                        - m_rho[0]*0.3);
        }
    } else if (grid(j) < m_zfixed) {
        rsd[index(c_offset_U_Le,j)] =
...
void FreeFlameLe::_finalize(const doublereal* x)
{
    StFlowLe::_finalize(x);
...
void FreeFlameLe::restore(const XML_Node& dom, doublereal* soln, int loglevel)
{
    StFlowLe::restore(dom, soln, loglevel);
    getOptionalFloat(dom, "t_fixed", m_tfixed);
    getOptionalFloat(dom, "z_fixed", m_zfixed);
}

XML_Node& FreeFlameLe::save(XML_Node& o, const doublereal* const sol)
{
    XML_Node& flow = StFlowLe::save(o, sol);
    if (m_zfixed != Undef) {
