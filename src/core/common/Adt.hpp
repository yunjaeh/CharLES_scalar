...
protected:
 ...
 public:
  ...
  void getBbox(T bbmin[3],T bbmax[3]) {
    // the global bounding box for all adt leaves is
    // stored in the first leaf (by construction)

    if (n == 0) {
      assert(leafdata == NULL);
      FOR_I3 bbmin[i] = getMaxLimit<T>();
      FOR_I3 bbmax[i] = -getMaxLimit<T>();
    }
    else {
      assert(n > 0);
      assert(leafdata);
      FOR_I3 bbmin[i] = leafdata[0].bbmin[i];
      FOR_I3 bbmax[i] = leafdata[0].bbmax[i];
    }
  ...
  void buildListForSphericalShell(std::vector<int>& bboxVec,const T xs[3],const T r0,const T r1); // greater than r0 and less than or equal to r1
  ...
  void dumpLeafData(const int ileaf) {
    ...
    if (n == 0) {
      std::cout << "no leaf data available on this instance of Adt " << std::endl;
      return;
    }
  ...
  double get_delta() const {
    ...
    assert(nleaves >= 1);
    return( max(leafdata[0].bbmax[0]-leafdata[0].bbmin[0],
		max(leafdata[0].bbmax[1]-leafdata[0].bbmin[1],
		    leafdata[0].bbmax[2]-leafdata[0].bbmin[2])) );
  }
...
template <class T>
void Adt<T>::buildListForSphericalShell(std::vector<int>& bboxVec,const T xs[3],const T r0,const T r1) {
