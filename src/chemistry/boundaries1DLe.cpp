...
void Bdry1D::_init(size_t n)
{
    ...
    if (m_index > 0) {
        ...
        if (r.domainType() == cFlowType) {
            ...
            throw CanteraError("Bdry1D::_init",
                               ...
                               "connected on the left to flow domains, not type "+std::to_string(r.domainType())
                               + " domains.");
    }
    ...
    if (m_index + 1 < container().nDomains()) {
        ...
        if (r.domainType() == cFlowType) {
            ...
            throw CanteraError("Bdry1D::_init",
                               "Boundary domains can only be "
                               "connected on the right to flow domains, not type "+std::to_string(r.domainType())
                               + " domains.");
    }
}
...
void Inlet1D::setMoleFractions(const std::string& xin)
{
    ...
    if (m_flow) {
        ...
        m_flow->phase().getMassFractions(&m_yin[0]);
        needJacUpdate();
    }
}
...
void OutletRes1D::setMoleFractions(const std::string& xres)
{
    ...
    if (m_flow) {
        ...
        m_flow->phase().getMassFractions(&m_yres[0]);
        needJacUpdate();
    }
}
                          ...
                          integer* diagg, doublereal rdt)
{
    ...
    m_sphase->setCoverages(&m_work[0]);
    ...
    m_kin->getNetProductionRates(&m_work[0]);
    ...
    if (m_flow_left) {
        nc = m_flow_left->nComponents();
        const doublereal* mwleft = &(m_phase_left->molecularWeights())[0];
        rb =r - nc;
        xb = x - nc;
        rb[2] = xb[2] - x[0];            // specified T
