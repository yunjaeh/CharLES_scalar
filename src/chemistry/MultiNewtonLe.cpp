            ...
            if (loglevel > 0) {
                if (val > above + 1.0e-12 || val < below - 1.0e-12) {
                    sprintf(buf, "domain %s: %20s(%s) = %10.3e (%10.3e, %10.3e)\n",
                            std::to_string(r.domainIndex()).c_str(),
                            r.componentName(m).c_str(), std::to_string(j).c_str(),
                            val, below, above);
    ...
    for (m = 0; m < NDAMP; m++) {
        ...
        if (loglevel > 0) {
            ...
            sprintf(m_buf,"\n%s  %9.5f   %9.5f   %9.5f   %9.5f   %9.5f %4d  %d/%d",
                    std::to_string(m).c_str(), damp, fbound, log10(ss+SmallNumber),
                       ...
                       OneDim& r, MultiJac& jac, int loglevel)
{
    ...
    while (1 > 0) {
        ...
        if (jac.age() > m_maxAge) {
            if (loglevel > 0)
              writelog("\nMaximum Jacobian age reached ("+std::to_string(m_maxAge)+")\n", loglevel);
            forceNewJac = true;
        }
        ...
        if (forceNewJac) {
            ...
            jac.updateTransient(rdt, &(r.transientMask())[0]);
            forceNewJac = false;
        }
