...
void updateCv2DataStart(T * s,const int action = REPLACE_DATA) {
...
void updateCv2DataFinish(T * s,const int action = REPLACE_DATA) {
    ...
    case MIN_DATA:
      {
        if (cvPrcomm) {
          for (int i = 0; i < cvPrcomm->unpack_size; ++i) {
            s[icv_g] = min(s[icv_g],mrs->UNPACK_BUF[unpack_size+i]);
            icv_g++;
          }
          unpack_size += cvPrcomm->unpack_size;
        }
        if (cv2Prcomm) {
          for (int i = 0; i < cv2Prcomm->unpack_size; ++i) {
            s[icv_g2] = min(s[icv_g2],mrs->UNPACK_BUF[unpack_size+i]);
            icv_g2++;
          }
          unpack_size += cv2Prcomm->unpack_size;
        }
    ...
    case REPLACE_DATA:
      {
        if (cvPrcomm) {
          for (int i = 0; i < cvPrcomm->unpack_size; ++i)
            s[icv_g++] = mrs->UNPACK_BUF[unpack_size+i];
          unpack_size += cvPrcomm->unpack_size;
        }
        if (cv2Prcomm) {
          for (int i = 0; i < cv2Prcomm->unpack_size; ++i)
            s[icv_g2++] = mrs->UNPACK_BUF[unpack_size+i];
          unpack_size += cv2Prcomm->unpack_size;
        }
      }
      break;
...
void updateCv2Data(T * s,const int action = REPLACE_DATA) {
  updateCv2DataStart(s,action);
  updateCv2DataFinish(s,action);
}
