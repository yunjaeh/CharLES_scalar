...
void InflowTurbulenceN::setStatistics() {
...
//      new comment: Keeping this here. Don't think it is needed.
        ...
        //      filename
        token = p->getString(iarg++);
        COUT2("  > using profiles of velocity, tke,C, Z from Fluent RANS file: " << token);
        //      read the profile
        FluentBPReader profile(token);
...
//Now compute the scalings
  ...
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
    ...
    solver->chemtable->lookup(&Rm, "R", &Tm, "T",  &gammam, "gamma", &Zm_it[ibf], &Zvarm_it[ibf], &Cm_it[ibf], 1);
...
void InflowTurbulenceN::updateBC(double (*FacePtrNew)[3],double (*FacePtrOld)[3]){
...
//  thermodynamic quantities
    ...
    assert(solver->chemtable != NULL);
    solver->chemtable->lookup(&R_bc, "R", &T_bc, "T", &e_bc, "e", &gamma_bc, "gamma", &Zm_it[ibf], &Zvarm_it[ibf], &Cm_it[ibf], 1);
...
int InflowTurbulenceN::solveVecBoundaryCvCg(double (*sol)[3],const double * const A,double (*rhs)[3], double * AddedDiag,double zero, int maxiter ) {
  bool verbose = false;
...
//            REDUNDANT CODE
///////////////////////////////////////////////////////////////////////////////////
  void InflowTurbulenceN::updateInflowTurbulence(double (*FacePtr)[3]){
...
//  update the boundary conditions. The turbulent field is stored in u_bc// should rename
  ...
  void InflowTurbulenceN::buildITBoundaryStructures() {
...
//  step one. Flag all boundary cvs that are adjecent to a wall
    for(vector<NonpremixedBc*>::iterator it = solver->bcs.begin(); it != solver->bcs.end(); ++it){
      string s=(*it)->getName();
      Param* p=getParam(s);
...
//This routine loops over all bfs and computes the distance based on all known near wll bfs. It may become exceedingly slow....
  void InflowTurbulenceN::setLengthscale() {
...
//    First we compute the distance to the wall in these cvs
  ...
  void InflowTurbulenceN::buildFilterOperators() {
//    MPI_Barrier(mpi_comm);
...
//  loop to compute l over dx and
  ...
  void InflowTurbulenceN::initiateStats() { // this must be smart enough to recognize a restart
//    MPI_Barrier(mpi_comm);
...
//    MPI_Barrier(mpi_comm);
  ...
  void InflowTurbulenceN::updateRandomField(double (*FacePtr)[3]) {
...
//  clean up
  ...
  void InflowTurbulenceN::removeMeanAndSetVarianceOne(double (*FacePtr)[3]) {
...
//      for the first steps the rms may be zero, so we need to check
  ...
  void InflowTurbulenceN::computeCholesky(double (*FacePtr)[3]) {
...
//    velocity component
  ...
  void InflowTurbulenceN::updateCvData_(double (*var_)[3]) {
...
//TODO This routine should be updated to use a local communicator to speed up the inflow turbulence
  void InflowTurbulenceN::updateCvData_(double *var_) {
...
//two pointers as input. Moves data from the cv pointer to the right face.
  void InflowTurbulenceN::getCvData_(double *FacePtr, double *CvPtr_){
    for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
...
//two pointers as input. Moves data from the cv pointer to the right face.
  void InflowTurbulenceN::getCvVectorData_(double (*FacePtr)[3], double (*CvPtr)[3]){
    for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
      FOR_I3{
