                          ...
                          doublereal curve, doublereal prune)
{
    if (ratio < 2.0) {
        throw CanteraError("Refiner::setCriteria",
            "'ratio' must be greater than 2.0 (" + std::to_string(ratio) +
            " was specified).");
    } else if (slope < 0.0 || slope > 1.0) {
        throw CanteraError("Refiner::setCriteria",
            "'slope' must be between 0.0 and 1.0 (" + std::to_string(slope) +
            " was specified).");
    } else if (curve < 0.0 || curve > 1.0) {
        throw CanteraError("Refiner::setCriteria",
            "'curve' must be between 0.0 and 1.0 (" + std::to_string(curve) +
            " was specified).");
    } else if (prune > curve || prune > slope) {
        throw CanteraError("Refiner::setCriteria",
            "'prune' must be less than 'curve' and 'slope' (" + std::to_string(prune) +
            " was specified).");
    }
    m_ratio = ratio;
    m_slope = slope;
...
{
    if (n >= m_npmax) {
        writelog("max number of grid points reached ("+std::to_string(m_npmax)+".\n");
        return -2;
    }
    ...
    FreeFlameLe* fflame = dynamic_cast<FreeFlameLe*>(m_domain);
...
void Refiner::show()
{
    if (!m_loc.empty()) {
        ...
        for (; b != m_loc.end(); ++b) {
            writelog(std::to_string(b->first)+" ");
        }
        writelog("\n");
        writelog("    to resolve ");
