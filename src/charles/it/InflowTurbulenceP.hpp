...
void InflowTurbulenceP::setStatistics() {
  Param * p = getParam(getName());
  assert(p->getString() == "INFLOW_TURB");
  int iarg = 1;
  while (iarg < p->size()) {
...
//  first is the profile
    if (token == "PROFILE") {
      ...
      if (mpi_rank == 0) cout << " > PROFILE: " << token << endl;
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//    stats from a hook
//    PROFILE HOOK
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//    PROFILE CONSTANT_UTKES <ux> <uy> <uz> <tke> <Z> <C>
//    NOTE: turbulent kinetic enery (tke) is defined as:
//          tke = 1/2 * (u'u'+v'v'+w'w')
//////////////////////////////////////////////////////////////////////////////////////////////////////////
      else if (token == "CONSTANT_UTKES") {
        double my_u_bc[3];
        my_u_bc[0] = p->getDouble(iarg++);
        my_u_bc[1] = p->getDouble(iarg++);
        my_u_bc[2] = p->getDouble(iarg++);
        double tke = p->getDouble(iarg++);
        double Zm  = p->getDouble(iarg++);
        double Cm  = p->getDouble(iarg++);
        if (mpi_rank == 0)
          cout  << "  > using constant mean velocity, constant turbulent kinetic energy, and constant Z and C:\n"
                << "\t(ux,uy,uz) = (" << my_u_bc[0] << "," << my_u_bc[1] << "," << my_u_bc[2] << ")\n"
                << "\ttke = " << tke <<"\n"
                << "\t(Z,C) = (" << Zm << "," << Cm << ")\n";
        ...
        cout << "I don't know what this was supposed to do so I'm not implementing it\n";
        throw(-1);
      }//"TRIP_CONSTANT_TKE_ADIABATIC"
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//    constant properties across the zone given mean square quantities
//    PROFILE CONSTANT_UMSS <ux> <uy> <uz> <u'u'> <v'v'> <w'w'> <Z> <C>
//////////////////////////////////////////////////////////////////////////////////////////////////////////
      else if (token == "CONSTANT_UMSS") {
        double my_u_bc[3];
        my_u_bc[0] = p->getDouble(iarg++);
        my_u_bc[1] = p->getDouble(iarg++);
        my_u_bc[2] = p->getDouble(iarg++);
        ...
        double Cm   = p->getDouble(iarg++);
        if (mpi_rank == 0)
          cout << "  > using constant mean velocity and constant turbulent mean squares:\n"
               << "\t(ux,uy,uz) = (" << my_u_bc[0] << "," << my_u_bc[1] << "," << my_u_bc[2] << ")\n"
               << "\t(uxux,uyuy,uzuz) = (" << my_uu_bc[0] << "," << my_uu_bc[1] << "," << my_uu_bc[2] << ")\n"
               << "\t(Z,C) = (" << Zm << "," << Cm << ")\n";
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          FOR_I3 um_it[ibf][i]  = my_u_bc[i];
...
//    PROFILE CONSTANT_UREYS <ux> <uy> <uz> <u'u'> <v'v'> <w'w'> <v'w'> <u'w'> <u'v'> <Z> <C>
//    and
//    Rd  = [<u'u'> <v'v'> <w'w'>]
//    Rod = [<v'w'> <u'w'> <u'v'>]
//////////////////////////////////////////////////////////////////////////////////////////////////////////
      else if (token == "CONSTANT_UREYS") {
        double my_u_bc[3];
        my_u_bc[0] = p->getDouble(iarg++);
        my_u_bc[1] = p->getDouble(iarg++);
        my_u_bc[2] = p->getDouble(iarg++);
        ...
        double Cm   = p->getDouble(iarg++);
        if (mpi_rank == 0)
          cout << "  > using constant mean velocity and constant Reynolds stresses:\n"
               << "\t(ux,uy,uz) = (" << my_u_bc[0] << "," << my_u_bc[1] << "," << my_u_bc[2] << ")\n"
               << "\t(uxux,uyuy,uzuz) = (" << my_uu_bc[0] << "," << my_uu_bc[1] << "," << my_uu_bc[2] << ")\n"
               << "\t(uxuy,uxuz,uyuz) = (" << my_uv_bc[0] << "," << my_uv_bc[1] << "," << my_uv_bc[2] << ")\n"
               << "\t(Z,C) = (" << Zm << "," << Cm << ")" << endl;
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          FOR_I3 um_it[ibf][i]  = my_u_bc[i];
          FOR_I3 Rd_it[ibf][i]  = my_uu_bc[i];
...
//    PROFILE FLUENT_UTKES
//    NOTE: turbulent kinetic enery (tke) is defined as:
//            tke = 1/2 * (u'u'+v'v'+w'w')
//////////////////////////////////////////////////////////////////////////////////////////////////////////
      else if (token == "FLUENT_UTKE_CONSTANT_S") {
        //      filename
        token = p->getString(iarg++);
        COUT2("  > using profiles of velocity and tke from Fluent RANS file: " << token << " plus constant Z and C values");
        //      read the profile
      ...
      }//"FLUENT_UTKE_CONSTANT_S"
      else if (token == "FLUENT_UREY_CONSTANT_S") {
        //      filename
        token = p->getString(iarg++);
        COUT2("  > using profiles of velocity, Reynolds stresses, p, and T from Fluent RANS file: " << token << " plus constant Z and C values");
        //      read the profile
        FluentBPReader profile(token);
        double Zm   = p->getDouble(iarg++);
        double Cm   = p->getDouble(iarg++);
        ...
        if (mpi_rank == 0) cout << "mean (Z,C), values are (" << Zm << ", " << Cm << ")\n";
        //      set the points
        //      old comment:
        //      set the points, they are contiguous in memory for now, but
        //      they might not always be ...
      ...
      }//"FLUENT_UREY_CONSTANT_S"
      else if (token == "FLUENT_UTKES") {
        //      filename
        token = p->getString(iarg++);
        COUT2("  > using profiles of velocity, tke, Z, C from Fluent RANS file: " << token);
        //      read the profile
        FluentBPReader profile(token);
        ...
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          ...
          um_it[ibf][1] = profile.getData(ibf,"y-velocity");
          um_it[ibf][2] = profile.getData(ibf,"z-velocity");
          Zm_it[ibf]    = profile.getData(ibf,"mixture-fraction");
          Cm_it[ibf]    = profile.getData(ibf,"progress-variable");
        }
      }//"FLUENT_UTKE_CONSTANT_S"
      else if (token == "FLUENT_UREYS") {
        //      filename
        token = p->getString(iarg++);
        COUT2("  > using profiles of velocity, Reynolds stresses, p, T, Z, C from Fluent RANS file: " << token);
        //      read the profile
        FluentBPReader profile(token);
      ...
      COUT2("  > maximum length scale " <<token <<" set to: " << max_length);
    }//"L_MAX"
    else if(token=="L"){
       token=p->getString(iarg++);
       if(token=="HOOK"){
...
//Now compute the scalings
  ...
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
    ...
    double Rm, Tm, gammam;
    assert(solver->chemtable != NULL);
    solver->chemtable->lookup(&Rm, "R", &Tm, "T",  &gammam, "gamma", &Zm_it[ibf], &Cm_it[ibf], 1);
    double this_sos= sqrt(gammam*Rm*Tm);
    assert(u_dot_A<=0.0);
...
void InflowTurbulenceP::updateBC(double (*FacePtrNew)[3],double (*FacePtrOld)[3]){
...
//  thermodynamic quantities
    ...
    double R_bc, T_bc, e_bc, rho_bc, gamma_bc, p_bc;
    assert(solver->chemtable != NULL);
    solver->chemtable->lookup(&R_bc, "R", &T_bc, "T", &e_bc, "e", &gamma_bc, "gamma", &Zm_it[ibf], &Cm_it[ibf], 1);
    ...
    bf_state[ibf].p      = p_bc;
//  Can add u_prime to it if nec...
    bf_state[ibf].h      = e_bc + p_bc/rho_bc + 0.5*(um_it[ibf][0]*um_it[ibf][0] + um_it[ibf][1]*um_it[ibf][1] + um_it[ibf][2]*um_it[ibf][2]);
    bf_state[ibf].Z      = Zm_it[ibf];
    bf_state[ibf].C      = Cm_it[ibf];
  }//(ibf < nbf)
}//updateBc()

// TODO This routine should be updated to use a local communicator to speed up the inflow turbulence
int InflowTurbulenceP::solveVecBoundaryCvCg(double (*sol)[3],const double * const A,double (*rhs)[3], double * AddedDiag,double zero, int maxiter ) {
  bool verbose = false;
...
//            REDUNDANT CODE
///////////////////////////////////////////////////////////////////////////////////
  void InflowTurbulenceP::updateInflowTurbulence(double (*FacePtr)[3]){
...
//  update the boundary conditions. The turbulent field is stored in u_bc// should rename
  ...
  void InflowTurbulenceP::buildITBoundaryStructures() {
//    MPI_Barrier(mpi_comm);
//    if (mpi_rank == 0) cout << "buildITBoundaryStructures()\n";
//    MPI_Barrier(mpi_comm);
...
//  step one. Flag all boundary cvs that are adjecent to a wall
    for(vector<PremixedBc*>::iterator it = solver->bcs.begin(); it != solver->bcs.end(); ++it){
      string s=(*it)->getName();
      Param* p=getParam(s);
...
//This routine loops over all bfs and computes the distance based on all known near wll bfs. It may become exceedingly slow....
  void InflowTurbulenceP::setLengthscale() {
    ...
    if(in_plane_length==1.0E+200) { // A constant lengthscale is not specified and we calculate the distance to the wall with a maximum threshold L_MAX
      double * wall_dist = new double[nbf];
      int ncv=solver->ncv;
...
//        TODO This must recognize all wall bcs. How?
  ...
  void InflowTurbulenceP::buildFilterOperators() {
//    MPI_Barrier(mpi_comm);
//    if (mpi_rank == 0) cout << "buildFilterOperators()\n";
//    MPI_Barrier(mpi_comm);
...
//  loop to compute l over dx and
  ...
  void InflowTurbulenceP::initiateStats() { // this must be smart enough to recognize a restart
//    MPI_Barrier(mpi_comm);
//    if (mpi_rank == 0) cout << "initiateStats()\n";
//    MPI_Barrier(mpi_comm);
  ...
  void InflowTurbulenceP::updateRandomField(double (*FacePtr)[3]) {
...
//  clean up
  ...
  void InflowTurbulenceP::removeMeanAndSetVarianceOne(double (*FacePtr)[3]) {
...
//      for the first steps the rms may be zero, so we need to check
  ...
  void InflowTurbulenceP::computeCholesky(double (*FacePtr)[3]) {
...
//    velocity component
  ...
  void InflowTurbulenceP::updateCvData_(double (*var_)[3]) {
...
//TODO This routine should be updated to use a local communicator to speed up the inflow turbulence
  void InflowTurbulenceP::updateCvData_(double *var_) {
...
//two pointers as input. Moves data from the cv pointer to the right face.
  void InflowTurbulenceP::getCvData_(double *FacePtr, double *CvPtr_){
    for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
...
//two pointers as input. Moves data from the cv pointer to the right face.
  void InflowTurbulenceP::getCvVectorData_(double (*FacePtr)[3], double (*CvPtr)[3]){
    for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
      FOR_I3{
