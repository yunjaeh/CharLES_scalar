    ...
    OneDim(domains)
{
    ...
    for (size_t n = 0; n < m_nd; n++) {
        domain(n)._getInitialSoln(&m_x[0] + start(n));
    }
...
void Sim1D::setValue(size_t dom, size_t comp, size_t localPoint,  doublereal value)
{
    ...
    AssertThrowMsg(iloc < m_x.size(), "Sim1D::setValue",
                   "Index out of bounds:" + std::to_string(iloc) + " > " +
                   std::to_string(m_x.size()));
    m_x[iloc] = value;
}
...
{
    OneDim::save(fname, id, desc, &m_x[0], loglevel);
}
                    ...
                    int loglevel)
{
    ...
    if (xd.size() != m_nd) {
        throw CanteraError("Sim1D::restore", "Solution does not contain the "
                           ...
                           std::to_string(xd.size()) + "expected " +
                           std::to_string(m_nd) + ".\n");
    }
    size_t sz = 0;
    ...
    m_xnew.resize(sz);
    for (size_t m = 0; m < m_nd; m++) {
        domain(m).restore(*xd[m], &m_x[0] + domain(m).loc(), loglevel);
    }
    resize();
    finalize();
}
...
void Sim1D::showSolution(ostream& s)
{
    for (size_t n = 0; n < m_nd; n++) {
        if (domain(n).domainType() != cEmptyType) {
            domain(n).showSolution_s(s, &m_x[0] + start(n));
        }
    }
}
...
void Sim1D::showSolution()
{
    for (size_t n = 0; n < m_nd; n++) {
        if (domain(n).domainType() != cEmptyType) {
            writelog("\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> "+domain(n).id()
                     +" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n\n");
            domain(n).showSolution(&m_x[0] + start(n));
        }
    }
}
...
{
    for (size_t n = 0; n < m_nd; n++) {
        domain(n)._finalize(&m_x[0] + start(n));
    }
}
...
{
    int m = OneDim::solve(&m_x[0], &m_xnew[0], loglevel);
    if (m >= 0) {
        ...
        throw CanteraError("Sim1D::newtonSolve",
                           "ERROR: OneDim::solve returned m = " + std::to_string(m) + "\n");
    }
}
...
void Sim1D::solve(int loglevel, bool refine_grid)
{
    ...
    while (new_points > 0) {
        ...
        bool ok = false;
        if (loglevel > 0) writeline('.', 78, true, true);
        while (!ok) {
            if (loglevel > 0) writelog("Attempt Newton solution of steady-state problem...", loglevel);
            ...
            if (status == 0) {
                if (loglevel > 0) {
                    ...
                    for (size_t mm = 1; mm < nDomains(); mm+=2) {
                        writelog(std::to_string(domain(mm).nPoints()));
                        if (mm + 2 < nDomains()) {
                            writelog(", ");
                        }
                    }
                ...
                if (loglevel > 0) writelog("Take "+std::to_string(nsteps)+" timesteps   ", loglevel);
                dt = timeStep(nsteps, dt, &m_x[0], &m_xnew[0], loglevel-1);
                if (loglevel > 6) {
...
int Sim1D::refine(int loglevel)
{
    ...
    for (size_t n = 0; n < m_nd; n++) {
        ...
        for (size_t m = 0; m < npnow; m++) {
            ...
            if (r.keepPoint(m)) {
                ...
                if (loglevel > 0) writelog("refine: discarding point at "+std::to_string(d.grid(m))+"\n", loglevel);
            }
        }
    ...
    for (size_t n = 0; n < m_nd; n++) {
        Domain1D& d = domain(n);
        gridsize = dsize[n];
        d.setupGrid(gridsize, &znew[0] + gridstart);
        gridstart += gridsize;
    }
...
int Sim1D::setFixedTemperature(doublereal t)
{
    ...
    for (n = 0; n < m_nd; n++) {
        ...
        FreeFlameLe* d_free = dynamic_cast<FreeFlameLe*>(&domain(n));
        size_t npnow = d.nPoints();
        size_t nstart = znew.size();
        if (d_free) {
            for (m = 0; m < npnow-1; m++) {
...
{
    OneDim::evalSSJacobian(&m_x[0], &m_xnew[0]);
}
}
