...

{
  ...
  const double d02 = (-PHIM0*DP0+PHIP0*DM0)/(PHIP0-PHIM0);
  const double d12 = (-PHIM1*DP0+PHIP0*DM1)/(PHIP0-PHIM1);
  const double d03 = (-PHIM0*DP1+PHIP1*DM0)/(PHIP1-PHIM0);
  const double d13 = (-PHIM1*DP1+PHIP1*DM1)/(PHIP1-PHIM1);

  triVec.push_back(pair<SimpleTriWithData,int> (SimpleTriWithData(x12,x02,x13,d12,d02,d13),MESH_EDGES0));
  triVec.push_back(pair<SimpleTriWithData,int> (SimpleTriWithData(x03,x13,x02,d03,d13,d02),MESH_EDGES1));
