...
void updateCv2DataStart(T (*s)[3],const int action = REPLACE_ROTATE_DATA) {
  ...
  while ((ii != cvPrcommVec.size())||(ii2 != cv2PrcommVec.size())) {
    ...
    if (cvPrcomm) {
      unpack_size += cvPrcomm->unpack_size*3;
      pack_size += cvPrcomm->packVec.size()*3;
    }
    if (cv2Prcomm) {
      unpack_size += cv2Prcomm->unpack_size*3;
      pack_size += cv2Prcomm->packVec.size()*3;
    }
    ++rank_count;
  }
  
  assert(rank_count == cv2PrcommVec.size()); // cv2PrcommVec has at least the ranks from cvPrcommVec
  assert(unpack_size == (ncv_g2-ncv)*3);
  ...
  while ((ii != cvPrcommVec.size())||(ii2 != cv2PrcommVec.size())) {
	...
	for (int jj = 0, limit = cvPrcomm->transformVec.size(); jj < limit; ++jj) {
	  if (cvPrcomm->transformVec[jj].has_R) {
	    for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
	      const int icv = cvPrcomm->packVec[i];
	      MiscUtils::matVecMult(mrs->PACK_BUF+pack_size+3*i,cvPrcomm->transformVec[jj].R,s[icv]);
	    }
	  }
	  else {
	    for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
	...
	break;
 	...
 	for (int jj = 0, limit = cvPrcomm->transformVec.size(); jj < limit; ++jj) {
          if ((cvPrcomm->transformVec[jj].has_R)&&(cvPrcomm->transformVec[jj].has_t)) {
            for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
              const int icv = cvPrcomm->packVec[i];
              double tmp[3];
              MiscUtils::matVecMult(tmp,cvPrcomm->transformVec[jj].R,s[icv]);
              MiscUtils::vecVecAdd(mrs->PACK_BUF+pack_size+3*i,cvPrcomm->transformVec[jj].t,tmp);
            }
          }
          else if (cvPrcomm->transformVec[jj].has_R) {
	    for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
	      const int icv = cvPrcomm->packVec[i];
	      MiscUtils::matVecMult(mrs->PACK_BUF+pack_size+3*i,cvPrcomm->transformVec[jj].R,s[icv]);
	    }
	  }
	  else if (cvPrcomm->transformVec[jj].has_t) {
	    for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
	      const int icv = cvPrcomm->packVec[i];
	      MiscUtils::vecVecAdd(mrs->PACK_BUF+pack_size+3*i,cvPrcomm->transformVec[jj].t,s[icv]);
	    }
	  }
	  else {
	    for (int i = cvPrcomm->transformVec[jj].start; i != cvPrcomm->transformVec[jj].end; ++i) {
	...
	for (int jj = 0, limit = cv2Prcomm->transformVec.size(); jj < limit; ++jj) {
	  if (cv2Prcomm->transformVec[jj].has_R) {
	    for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
	      const int icv = cv2Prcomm->packVec[i];
	      MiscUtils::matVecMult(mrs->PACK_BUF+pack_size+3*i,cv2Prcomm->transformVec[jj].R,s[icv]);
	    }
	  }
	  else {
	    for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
	...
	break;
      case REPLACE_TRANSLATE_DATA:
 	for (int jj = 0, limit = cv2Prcomm->transformVec.size(); jj < limit; ++jj) {
          if ((cv2Prcomm->transformVec[jj].has_R)&&(cv2Prcomm->transformVec[jj].has_t)) {
            for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
              const int icv = cv2Prcomm->packVec[i];
              double tmp[3];
              MiscUtils::matVecMult(tmp,cv2Prcomm->transformVec[jj].R,s[icv]);
              MiscUtils::vecVecAdd(mrs->PACK_BUF+pack_size+3*i,cv2Prcomm->transformVec[jj].t,tmp);
            }
          }
          else if (cv2Prcomm->transformVec[jj].has_R) {
	    for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
	      const int icv = cv2Prcomm->packVec[i];
	      MiscUtils::matVecMult(mrs->PACK_BUF+pack_size+3*i,cv2Prcomm->transformVec[jj].R,s[icv]);
	    }
	  }
	  else if (cv2Prcomm->transformVec[jj].has_t) {
	    for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
	      const int icv = cv2Prcomm->packVec[i];
	      MiscUtils::vecVecAdd(mrs->PACK_BUF+pack_size+3*i,cv2Prcomm->transformVec[jj].t,s[icv]);
	    }
	  }
	  else {
	    for (int i = cv2Prcomm->transformVec[jj].start; i != cv2Prcomm->transformVec[jj].end; ++i) {
...
void updateCv2DataFinish(T (*s)[3]) {
  ...
  while ((ii != cvPrcommVec.size())||(ii2 != cv2PrcommVec.size())) {
	...
	s[icv_g][0] = mrs->UNPACK_BUF[unpack_size+3*i  ];
	s[icv_g][1] = mrs->UNPACK_BUF[unpack_size+3*i+1];
	s[icv_g][2] = mrs->UNPACK_BUF[unpack_size+3*i+2];
        ++icv_g;
      }
      unpack_size += cvPrcomm->unpack_size*3;
    }
	...
	s[icv_g2][0] = mrs->UNPACK_BUF[unpack_size+3*i  ];
	s[icv_g2][1] = mrs->UNPACK_BUF[unpack_size+3*i+1];
	s[icv_g2][2] = mrs->UNPACK_BUF[unpack_size+3*i+2];
        ++icv_g2;
      }
      unpack_size += cv2Prcomm->unpack_size*3;
    }
...
void updateCv2Data(T (*s)[3],const int action = REPLACE_ROTATE_DATA) {
...
// =====================================================================
// OLD version: get rid of these eventually
//
// these routines use the mpi_comm to exchange data... 
// old versions that only do level-2....
// =====================================================================

void updateCv2DataStartOld(T * s) {
  ...
  mrs->sendRequestArray = new MPI_Request[cv2PrcommVec.size()];
  mrs->recvRequestArray = new MPI_Request[cv2PrcommVec.size()];
  //mrs->statusArray = new MPI_Status[cv2PrcommVec.size()];
    
  //int unpack_size = 0;
  int pack_size = 0;
  for (int ii = 0; ii < cv2PrcommVec.size(); ++ii) {
    //unpack_size += cv2PrcommVec[ii].unpackVec.size();
    pack_size += cv2PrcommVec[ii].packVec.size();
  }
  ...
  for (int ii = 0; ii < cv2PrcommVec.size(); ++ii) {
    ...
    MPI_Irecv(s+cv2PrcommVec[ii].unpack_offset,cv2PrcommVec[ii].unpack_size,
	      MPI_T,cv2PrcommVec[ii].rank,UPDATE_TAG,mpi_comm,&(mrs->recvRequestArray[ii]));
      
    //unpack_size += cv2PrcommVec[ii].unpackVec.size();
    
    // pack and send...
    
    for (int i = 0; i < cv2PrcommVec[ii].packVec.size(); ++i) {
      const int icv = cv2PrcommVec[ii].packVec[i];
      assert((icv >= 0)&&(icv < ncv));
      mrs->PACK_BUF[pack_size+i] = s[icv];
    }
    
    MPI_Issend(mrs->PACK_BUF+pack_size,cv2PrcommVec[ii].packVec.size(),
	       MPI_T,cv2PrcommVec[ii].rank,UPDATE_TAG,mpi_comm,&(mrs->sendRequestArray[ii]));
...
void updateCv2DataFinishOld(T * s) {
...
void updateCv2DataOld(T * s) {
  updateCv2DataStartOld(s);
  updateCv2DataFinishOld(s);
}

void updateCv2DataStartOld(T (*s)[3]) {
  ...
  int pack_size = 0;
  for (int ii = 0; ii < cv2PrcommVec.size(); ++ii) {
    //unpack_size += cv2PrcommVec[ii].unpackVec.size()*3;
    pack_size += cv2PrcommVec[ii].packVec.size()*3;
  }
  ...
  for (int ii = 0; ii < cv2PrcommVec.size(); ++ii) {
    ...
    // post irecv...
    MPI_Irecv(s+cv2PrcommVec[ii].unpack_offset,cv2PrcommVec[ii].unpack_size*3,
	      MPI_T,cv2PrcommVec[ii].rank,UPDATE_TAG,mpi_comm,&(mrs->recvRequestArray[ii]));
    ...
    MPI_Issend(mrs->PACK_BUF+pack_size,cv2PrcommVec[ii].packVec.size()*3,
	       MPI_T,cv2PrcommVec[ii].rank,UPDATE_TAG,mpi_comm,&(mrs->sendRequestArray[ii]));
...
void updateCv2DataFinishOld(T (*s)[3]) {
